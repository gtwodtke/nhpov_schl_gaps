...
################################################
##                                            ##
## PROGRAM NAME: 22_create_figure_C2.R        ##
##                                            ##
## PURPOSE: plot FAMD variance explained      ##
##                                            ##
################################################
################################################
...
##### PRINT OUTPUT #####
sink("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\programs\\_LOGS\\22_create_figure_C2_log.txt")

##### FACTOR ANALYSIS OF MIXED DATA (FAMD) ##### 
famd <- FAMD(eclsk, graph = FALSE, ncp = 10)
eig.val <- get_eigenvalue(famd)

print(eig.val)

##### CREATE PLOT #####
jpeg("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\figures\\figure_C2.jpeg",
  height=5,
  width=5,
  units='in',
  res=600)

f <- fviz_eig(famd, choice = c("variance"), ncp = 10) 
f + scale_y_continuous(labels = scales::percent_format(scale = 1), limits=c(0, 10)) +
  geom_bar(stat="identity", fill="gray40", color="black") +
