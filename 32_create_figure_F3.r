...
eclsk.mi$hipovnh<-ifelse(eclsk.mi$nhpovrt1>=0.30, 1, 0)

##### DROP OBS IN NHOODS W/ MODERATE POVERTY LEVELS #####
eclsk.mi <- eclsk.mi[which(eclsk.mi$nhpovrt1>=0.30 | eclsk.mi$nhpovrt1<=0.05),]

##### DEFINE CENSORING FUNCTION #####
censor <- function(x, low=0.05, high=0.95) {
  min <- quantile(x, low)
  max <- quantile(x, high)
...
mi.results <- foreach(i=1:nmi, .combine=cbind) %dopar% {
  ...
  var.unx.rd <- lm_robust((summand.rd.efx - rdtheta7)~1, cluster=schlid, se_type="stata", data=eifEst)$std.error^2
  ...
  var.unx.mt <- lm_robust((summand.mt.efx - mththeta7)~1, cluster=schlid, se_type="stata", data=eifEst)$std.error^2
...
##### PRINT RESULTS #####
sink("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\programs\\_LOGS\\32_create_figure_F3_log.txt")
...
plot.rd <- 
  ggplot(output.rd, aes(y=label, x=estimate, xmin=ll.pct.95ci, xmax=ul.pct.95ci)) +
    ...
    scale_x_continuous(limits=c(-1.0, 0.2), breaks=seq(-1.0, 0.2, 0.2), labels=function(x) format(round(x,2), nsmall=1, scientific=FALSE)) +
    labs(
