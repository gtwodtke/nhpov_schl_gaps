...
mi.results <- foreach(i=1:nmi, .combine=cbind) %dopar% {
  ...
  eifEst$phat.x1 <- eifEst$phat.x1.base <- eifEst$phat.x1.base.efx <- NA
  eifEst$yhat.x1.base.rd.efx <- eifEst$yhat.x1.base.mt.efx <- NA
  eifEst$yhat.x1.x0.base.rd.efx <- eifEst$yhat.x1.x0.base.mt.efx <- NA
  eifEst$yhat.x1.x0.base.x1.rd.efx <- eifEst$yhat.x1.x0.base.x1.mt.efx <- NA
  ...
  for (j in 1:kfolds) {
    ...
    nhpov.mod.base <- SuperLearner(
      ...
      X=eclsk[which(eclsk$k!=j), vars.base.adj],
      family = binomial(),
    ...
    nhpov.mod.base.efx <- SuperLearner(
      Y=eclsk[which(eclsk$k!=j), "hipovnh"],
      X=eclsk[which(eclsk$k!=j), c(vars.base.adj, vars.schl.all)],
      family = binomial(),
      SL.library=c("SL.glm", "customRF_1", "customGBTree_1"),
      cvControl=cntrl.sl.bin)
    
    eifEst[which(eifEst$k==j), "phat.x1"] <- mean(eclsk[which(eclsk$k!=j), "hipovnh"])
    eifEst[which(eifEst$k==j), "phat.x1.base"] <- predict(nhpov.mod.base, newdata = eclsk[which(eclsk$k==j), vars.base.adj])$pred
    eifEst[which(eifEst$k==j), "phat.x1.base.efx"] <- predict(nhpov.mod.base.efx, newdata = eclsk[which(eclsk$k==j), c(vars.base.adj, vars.schl.all)])$pred
    
    read.mod.base.efx <- SuperLearner(
      Y=eclsk[which(eclsk$k!=j), "rdtheta7"],
      X=eclsk[which(eclsk$k!=j), c("hipovnh", vars.base.adj, vars.schl.all)],
    ...
    eifEst[which(eifEst$k==j), "yhat.x1.base.rd.efx"] <- predict(read.mod.base.efx, newdata = eclsk.x1[which(eclsk.x1$k==j), c("hipovnh", vars.base.adj, vars.schl.all)])$pred
    eifEst[which(eifEst$k==j), "yhat.x1.base.mt.efx"] <- predict(math.mod.base.efx, newdata = eclsk.x1[which(eclsk.x1$k==j), c("hipovnh", vars.base.adj, vars.schl.all)])$pred
    
    eclsk[which(eclsk$k!=j), "yhat.x1.base.rd.efx"] <- predict(read.mod.base.efx, newdata = eclsk.x1[which(eclsk.x1$k!=j), c("hipovnh", vars.base.adj, vars.schl.all)])$pred
    eclsk[which(eclsk$k!=j), "yhat.x1.base.mt.efx"] <- predict(math.mod.base.efx, newdata = eclsk.x1[which(eclsk.x1$k!=j), c("hipovnh", vars.base.adj, vars.schl.all)])$pred
    
    e.read.mod.base.efx <- SuperLearner(
      Y=eclsk[which(eclsk$k!=j), "yhat.x1.base.rd.efx"],
      X=eclsk[which(eclsk$k!=j), c("hipovnh", vars.base.adj)],
      SL.library=c("SL.lm", "customRF_1", "customGBTree_1"),
      cvControl=cntrl.sl.cnt)
    ...
    eifEst[which(eifEst$k==j), "yhat.x1.x0.base.rd.efx"] <- predict(e.read.mod.base.efx, newdata = eclsk.x0[which(eclsk.x0$k==j), c("hipovnh", vars.base.adj)])$pred
    eifEst[which(eifEst$k==j), "yhat.x1.x0.base.mt.efx"] <- predict(e.math.mod.base.efx, newdata = eclsk.x0[which(eclsk.x0$k==j), c("hipovnh", vars.base.adj)])$pred
    
    eclsk[which(eclsk$k!=j), "yhat.x1.x0.base.rd.efx"] <- predict(e.read.mod.base.efx, newdata = eclsk.x0[which(eclsk.x0$k!=j), c("hipovnh", vars.base.adj)])$pred
    eclsk[which(eclsk$k!=j), "yhat.x1.x0.base.mt.efx"] <- predict(e.math.mod.base.efx, newdata = eclsk.x0[which(eclsk.x0$k!=j), c("hipovnh", vars.base.adj)])$pred
  ...
  eifEst$wt1.efx[eifEst$hipovnh==0] <- 0
...
mi.results <- matrix(unlist(mi.results), ncol=2, byrow=TRUE)

##### COMBINE MI ESTIMATES #####
bias.comb <- matrix(data=NA, nrow=ncol(mi.results), ncol=1)

for (i in 1:nrow(bias.comb)) {
  bias.comb[i,1] <- round(mean(mi.results[,i]), digits=3)
}

##### COMPUTE BIAS TERMS #####
load("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\data\\_TEMP\\tbl1Output.RData")

bias.rd <- bias.comb[1,1]-output.rd["Elim. by Efx", "estimate"]
bias.mt <- bias.comb[2,1]-output.mt["Elim. by Efx", "estimate"]

##### COMPUTE BIAS-ADJUSTED ESTIMATES #####
psi <- seq(from=0, to=2, by=0.1)
bias.adj.rd <- bias.adj.mt <- matrix(data=NA, nrow=length(psi), ncol=4)

for (i in 1:length(psi)) {

  bias.adj.rd[i,1] <- psi[i]
  bias.adj.rd[i,2] <- output.rd["Elim. by Efx", "estimate"] - psi[i]*bias.rd
  bias.adj.rd[i,3] <- output.rd["Elim. by Efx", "ll.pct.95ci"] - psi[i]*bias.rd
  bias.adj.rd[i,4] <- output.rd["Elim. by Efx", "ul.pct.95ci"] - psi[i]*bias.rd

  bias.adj.mt[i,1] <- psi[i]
  bias.adj.mt[i,2] <- output.mt["Elim. by Efx", "estimate"] - psi[i]*bias.mt
  bias.adj.mt[i,3] <- output.mt["Elim. by Efx", "ll.pct.95ci"] - psi[i]*bias.mt
  bias.adj.mt[i,4] <- output.mt["Elim. by Efx", "ul.pct.95ci"] - psi[i]*bias.mt
}

##### PRINT RESULTS #####
clabel <- c("psi", "est", "ll95ci", "ul95ci")
colnames(bias.adj.rd) <- colnames(bias.adj.mt) <- clabel

sink("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\programs\\_LOGS\\16_create_figure_7_log.txt")

cat("READING: Bias-adjusted Disparity Eliminated \n")
print(bias.adj.rd)
cat(" \n")
cat("MATH: Bias-adjusted Disparity Eliminated \n")
print(bias.adj.mt)
cat(" \n")
...
##### PLOT RESULTS #####
#load("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\data\\_TEMP\\fig7Output.RData")

bias.adj.rd <- as.data.frame(bias.adj.rd)
plot.rd <- ggplot(bias.adj.rd, aes(x = psi)) +
                 geom_hline(yintercept = 0, color = "grey") +
                 geom_line(aes(y = ll95ci), linetype = "dashed") +
                 geom_line(aes(y = ul95ci), linetype = "dashed") +
                 geom_line(aes(y = est), linetype = "solid") +
                 labs(
                   title = "Reading Scores",
                   x = "Multiples of Psi",
                   y = "Bias-adjusted Estimate of Disparity Eliminated") +
                 xlim(0, 2) +
                 ylim(-0.2, 0.2) +
                 theme_bw() +
                 theme(plot.title = element_text(size=11.75))

bias.adj.mt <- as.data.frame(bias.adj.mt)
plot.mt <- ggplot(bias.adj.mt, aes(x = psi)) +
                 geom_hline(yintercept = 0, color = "grey") +
                 geom_line(aes(y = ll95ci), linetype = "dashed") +
                 geom_line(aes(y = ul95ci), linetype = "dashed") +
                 geom_line(aes(y = est), linetype = "solid") +
...
jpeg("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\figures\\figure_7.jpeg",
     height=5,
     width=9,
     units='in',
     res=600)

grid.arrange(plot.rd, plot.mt, ncol=2, nrow=1)
...
save(
  bias.adj.rd,
  bias.adj.mt,
  file="C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\data\\_TEMP\\fig7Output.RData")
