...
##          based on multidem nhdadvg index   ##
##                                            ##
################################################
################################################
...
pca.nh <- prcomp(eclsk.mi[, c("nhpovrt1","nhlesshs1","nhcolgrd1","nhfemhd1","nhunemprt1","nhshrwht1")], scale=TRUE)
eclsk.mi$hipovnh <- ifelse(pca.nh$x[,1] < quantile(pca.nh$x[,1], 0.2), 1, 0)
...
##### PRINT RESULTS #####
sink("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\programs\\_LOGS\\18_create_figure_A1_log.txt")
...
plot.rd <- 
  ggplot(output.rd, aes(y=label, x=estimate, xmin=ll.pct.95ci, xmax=ul.pct.95ci)) +
    ...
    scale_x_continuous(limits=c(-0.7, 0.1), breaks=seq(-0.7, 0.1, 0.1), labels=function(x) format(round(x,2), nsmall=1, scientific=FALSE)) +
    labs(
