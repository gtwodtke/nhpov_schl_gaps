...
mi.results <- foreach(i=1:nmi, .combine=cbind) %dopar% {
  ...
  var.unx.rd <- lm_robust((summand.rd.efx - rdtheta9)~1, cluster=schlid, se_type="stata", data=eifEst)$std.error^2
  ...
  var.unx.mt <- lm_robust((summand.mt.efx - mththeta9)~1, cluster=schlid, se_type="stata", data=eifEst)$std.error^2
...
##### PRINT RESULTS #####
sink("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\programs\\_LOGS\\25_create_figure_E2_log.txt")
...
plot.rd <- 
  ggplot(output.rd, aes(y=label, x=estimate, xmin=ll.pct.95ci, xmax=ul.pct.95ci)) +
    ...
    labs(
      title="5th Grade Reading Scores",
...
plot.mt <- 
  ggplot(output.mt, aes(y=label, x=estimate, xmin=ll.pct.95ci, xmax=ul.pct.95ci)) +
    ...
    scale_x_continuous(limits=c(-0.6, 0.1), breaks=seq(-0.6, 0.1, 0.1), labels=function(x) format(round(x,2), nsmall=1, scientific=FALSE)) +
    labs(
      title="5th Grade Math Scores",
      x="Standard Deviations",
      y=" ") +
    theme_bw()

jpeg("C:\\Users\\Geoffrey Wodtke\\Desktop\\projects\\nhood_schl_gaps\\figures\\figure_E2.jpeg", 
     height=5, 
     width=9,
     units='in',
     res=600)
